...
from app.domain.document.schemas import DocumentCreate, DocumentRead, DocumentPage, DocumentUpdate
...
def create_document(payload: DocumentCreate, svc: DocumentService = Depends(get_service)):
    return svc.create(payload)

@router.put("/<built-in function id>", response_model=DocumentRead)
def update_document(id: int, payload: DocumentUpdate, svc: DocumentService = Depends(get_service)):
